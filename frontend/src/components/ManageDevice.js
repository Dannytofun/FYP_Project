...
class ManageDevice extends Component {
  ...
  async updateDeviceData() {
    ...
    async function getDevices(instance, deviceId){
    ...
    async function getSignature(instance, deviceId){
      const getSig = new Promise(function(resolve, reject){
        let res = instance.deviceSignatureCount(deviceId, function(error, result){
          if(!error){
            resolve(result)
          }else{
            reject(error)
    ...
    try {
      ...
      let device = await getDevices(instance, deviceId);
      let signatureCount = await getSignature(instance, deviceId);
