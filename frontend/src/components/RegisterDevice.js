...
import { getDefaultAccount } from '../DeviceManager';
...
class RegisterDevice extends Component {
  ...
  async createDevice() {
    ...
    try {
      ...
      let tx;
      let self = this;
      let deviceCreatedEvent
      let res = await instance.createDevice(addHexPrefix(identifierToSave), addHexPrefix(metadataHash), addHexPrefix(firmwareHash), { from: user },function (error, result){
        if(!error){
          deviceCreatedEvent = instance.DeviceCreated()
          const txHash = result
          deviceCreatedEvent.watch((error, result) => {
            if (!error) {
              if (result.transactionHash === txHash) {
