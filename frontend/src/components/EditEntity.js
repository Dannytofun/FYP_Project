...
class EditEntity extends Component {
  ...
  async updateMyData() {
    try {
      ...
      console.log(account) 
      let res = await this.state.instance.ownerToEntity(account, function(error, result){
        if(!error){
          this.setState({
            myData: result,
            myDataNew: result,
  ...
  async saveMyData() {
    try {
      if (this.state.myDataNew !== this.state.myData) {
        let instance = await DeviceManager;
        await instance.updateEntityData(this.state.myDataNew, { from: getDefaultAccount() }, function (error, result){
          if(!error){
